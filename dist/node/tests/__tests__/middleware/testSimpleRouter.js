...
const SimpleRouter = index_1.Middleware.SimpleRouter;
...
describe('Simple Router Tests', () => {
    ...
    test('test addRoute adds values to the list of routes in the right format', () => {
        ...
        const path = '/';
        const method = 'GET';
        const fn = () => { return Promise.resolve(new index_2.Response('')); };
    ...
    test('test middleware function routes successfully to successful routes', () => {
        ...
        const fn = () => {
            route_hit = true;
            return Promise.resolve(new index_2.Response('{"route_hit": true}'));
        };
        const next = () => {
            return Promise.resolve(new index_2.Response(''));
        };
        router.addRoute(method, path, fn);
    ...
    test('test route successfully returns value out of the middleware', () => {
        ...
        const method = 'GET';
        const fn = () => {
            return Promise.resolve(new index_2.Response('route'));
        };
        router.addRoute(method, path, fn);
    ...
    test('test next successfully returns value out of the middleware', () => {
        ...
        const method = 'GET';
        const fn = () => {
            return Promise.resolve(new index_2.Response('true'));
        };
        router.addRoute(path, 'PUT', fn);
        const route_hit = router['_middleware'](router['_routes'], method, path, '', {}, () => {
            return Promise.resolve(new index_2.Response('next'));
        });
        route_hit.then((response) => {
    ...
    test('test routes loaded from a file', () => {
        ...
        return router.addRouteFile(router_file_test_path)
            ...
            return Promise.all([
                ...
                expect(results[0]).toEqual(new index_2.Response('root get successful'));
                expect(results[1]).toEqual(new index_2.Response('root post successful'));
                expect(results[2]).toEqual(new index_2.Response('test get successful'));
                expect(results[3]).toEqual(new index_2.Response('Page not found', 404));
                expect(results[4]).toEqual(new index_2.Response('Page not found', 404));
            });
        });
    });
