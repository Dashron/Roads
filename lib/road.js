 ...
 *
 * You must provide an array of root resource to the constructor. We will check each resource in array order for a matching route. The root resources will handle all requests to the root (`/`) endpoint. Any additional routes will be referenced as sub-resources of the root endpoint.
 * 
...
var Road = module.exports = function Road (root_resources) {
	...
	if (!this.root_resources) {
		throw new Error('You must configure at least one root resource before making requests');
	}
...
Road.prototype._buildRoute = function (request_method, parsed_url, request_body, request_headers) {
	...
	for (var i = 0; i < this.root_resources.length; i++) {
		...
		if (resource && resource[request_method]) {
	...
	return _self._buildRequestChainQueue(this._buildContext(resource), route, request_method, parsed_url, request_body, request_headers);
};
 ...
 * @return {Object}
 */
Road.prototype._buildContext = function (resource) {
	var _self = this;
	var context = {
