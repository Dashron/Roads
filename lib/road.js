...
Road.prototype._onRequest = null;

/**
 * Assign an on request handler
 * 
 * You must provide a callback to the onRequest function. This callback will be called any time a request is made, whether or not the route was successful.
 *
 ...
 */
Road.prototype.onRequest = function (fn) {
	this._onRequest = fn;
};
 ...
 * @return function
 */
Road.prototype._buildRoute = function (parsed_url, request_method, request_body, request_headers) {
	var _self = this;
	var next = null;
	var next_route = null;
	...
	if (this._onRequest) {
		next_route = _self._locateRoute(resource, parsed_url, request_method)
			.bind(context, parsed_url, request_body, request_headers);
		
		next = _self._executeRoute.bind(_self, next_route);

		return this._createCoroutine(this._onRequest)
			.bind(context, request_method, parsed_url, request_body, request_headers, next);
	} else {
		return this._locateRoute(resource, parsed_url, request_method)
			.bind(context, parsed_url, request_body, request_headers, next);
	}
};
 ...
 * @return function
 */
Road.prototype._locateRoute = function (resource, parsed_url, method) {
