 ...
 */
var Server = module.exports = function Server (road, error_handler) {
	if (!road) {
 ...
 * @type HTTPServer
 */
Server.prototype._server = null;

/**
 * This is the road object that will handle all requests
 * @type Road
 */
Server.prototype._road = null;
 ...
 * @type null|function
 */
Server.prototype._custom_error_handler = null;
 ...
 * @param  Error error
 */
Server.prototype._error_handler = function (http_response, error) {
 ...
 */
Server.prototype._writeToResponse = function (http_response, response) {
 ...
 */
Server.prototype._onRequest = function (http_request, http_response) {
	var body = '';
	var _self = this;
	...
	http_request.on('end', function () {
		...
		_self._road.request(http_request.method, http_request.url, body, http_request.headers)
			.then(success_handler).catch(function (err) {
				error_handler(err);
			}).catch(function (err) {
