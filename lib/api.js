 ...
 * @param  {object} context an optional context that will become ```this``` within each API request. A ```request``` method will be added to this object, and will override anything you provide. This method will be an alias for API.request.
 *
 *
 */
var API = module.exports = function API (root_resource, context) {
	var _self = this;
	...
	this._context.request = function (method, url, body, headers, extras) {
		return _self.request(method, url, body, headers, extras);
	};

	this._initResources(this._context, root_resource);
};

API.prototype.root_resource = null;
API.prototype._onError = null;
API.prototype._onRequest = null;
API.prototype._context = null;

API.prototype._initResources = function (context, resource) {
	if (resource._init) {
		resource._init.call(context);
	}

	for(var key in resource.routes) {
		if (resource.routes[key]) {
			this._initResources(context, resource.routes[key]);
		}
	}
};
 ...
 * **NOTE:** The response data will already be processed at this point through getData() and FieldsFilter. You should reference response.data directly, and not use getData().
 *
 * 
 * @param  string method
 * @param  string url     
...
API.prototype.request = function (method, url, body, headers) {
	...
	if (this._onRequest) {
		...
		route = this._createCoroutine(this._onRequest).bind(this._context, method);
	} else {
...
API.prototype._locateRoute = function (parsed_url, method) {
	...
	return this._createCoroutine(resource[method]).bind(this._context);
};
...
API.prototype._executeRoute = function (route, url, body, headers, extra_data) {
	...
	try {
		result = route.call(this._context, url, body, headers, extra_data);
	} catch (e) {
