...
API.prototype.locateResource = function (parsed_url) {
	...
	while (parts.length) {
		part = parts.shift();
		resourceKey = resource.getResourceKey(part);

		if (!resourceKey) {
			return false;
		} else {
			resource = resource.routes[resourceKey];
			// apply the route parts to the url arguments field
			if (resourceKey[0] === '$' || resourceKey[0] === '#') {
				parsed_url.args[resourceKey.substring(1)] = part;
			}
		}
	}
